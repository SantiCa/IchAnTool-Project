...
def OpenImg(root):
    ...
    global State
    import os
    config.rutaenlace=filedialog.askopenfilename(initialdir = "/",title = "Select file (jpg)")
...
########2 (getdata) 
    def getdata(Inicio,Fin,NameArchive,frame):
        ...
        carpeta=filedialog.askdirectory(title ="select where to save the CSV archive")
        if carpeta=="":
